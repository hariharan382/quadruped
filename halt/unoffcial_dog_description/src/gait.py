...
class gait:
    ...
    def joystick_callback(self,joy_status):
        ...
        #print("front_back: ",round(front_back),"right_left :",round(right_left))
        #print(r)
        ...
        if abs(r):
            ...
            self.theta=0
            #print("rotate-phase")
        elif round(front_back) !=0 or round(right_left) !=0:
            self.halt = 0
            self.rotate = 0
            self.theta=np.arctan2(right_left,front_back)* 180 / np.pi
            #print("moving-phase       --------     theta_value :",self.theta)
            
        elif round(front_back) == 0 & round(right_left) == 0:
            self.halt = 1
            self.rotate = 0
            #print("halt-phase")
            #print("halt_value : --",self.halt)
    ...
    def walk(self):
        ...
        LF=np.array([h,-2,z_off,1])
        RF=np.array([h,2,z_off,1])
        RB=np.array([h,2,z_off,1])
        LB=np.array([h,-2,z_off,1])
        ...
        while not rospy.is_shutdown():
            ...
            if self.halt != 1 and not abs(self.rotate):
                ...
                if self.swing == 0:
                    ...
                    while(t<T):
                        ...
                        if(30+degrees(RB_J2) and 30+degrees(RF_J2) > 0):
                            ...
                            ard_RB1.publish(np.int32(90+degrees(RB_J1)+first))
                            ...
                            ard_RF1.publish(np.int32(90+degrees(RF_J1)+first))
                            ard_RF2.publish(np.int32(degrees(RF_J2)+30+10))
                ...
                if self.swing==1:
                    ...
                    while (t < T):
                        ...
                        if(180-degrees(RB_J2) and 180-degrees(RF_J2)<=180):
                            ...
                            #LB_List.append(degrees(LB_J2))
                            #LF_List.append(degrees(LF_J2))
                            ard_LB1.publish(np.int32(90-degrees(LB_J1)-first)) #ard_LB1.publish(np.int32(90-degrees(90)))
                            ard_LB2.publish(np.int32(150-degrees(LB_J2)))
                            ard_LB3.publish(np.int32(degrees(LB_J3)-45+degrees(LB_J2)-h_off))
                            ard_LF1.publish(np.int32(90-degrees(LF_J1)-first))
                            ard_LF2.publish(np.int32(150-degrees(LF_J2)))
            ...
            if self.halt==1:
                ...
                ard_LB2.publish(np.int32(100))
                ard_LB3.publish(np.int32(95))
                ard_LF1.publish(np.int32(90))
                ard_LF2.publish(np.int32(100))
                ard_LF3.publish(np.int32(70))

                ard_RB1.publish(np.int32(90)) #ard_LB1.publish(np.int32(90-degrees(90)))
                ard_RB2.publish(np.int32(80))
                ard_RB3.publish(np.int32(95))
                ard_RF1.publish(np.int32(90))
                ard_RF2.publish(np.int32(80))
                ard_RF3.publish(np.int32(90))  """
                eff=-10
                off=-10
                front_height=-10
                thigh=10
                ...
                thigh_angle=45+5
                knee_angle=90-10
                ard_LB1.publish(np.int32(90-eff-adduction_angle)) #ard_LB1.publish(np.int32(90-degrees(90)))
                ard_LB2.publish(np.int32(150-thigh_angle-lb2_eff+15))
                ard_LB3.publish(np.int32(thigh_angle+knee_angle-60))
                ard_LF1.publish(np.int32(80-eff-adduction_angle))
                ard_LF2.publish(np.int32(150-thigh_angle-5))
                ard_LF3.publish(np.int32(thigh_angle+knee_angle-60))
                

                ard_RB1.publish(np.int32(90+eff+adduction_angle)) #ard_LB1.publish(np.int32(90-degrees(90)))
                ard_RB2.publish(np.int32(30+thigh_angle-5))
                ard_RB3.publish(np.int32(180-thigh_angle-knee_angle+60))
                ard_RF1.publish(np.int32(90+eff+adduction_angle))
                ard_RF2.publish(np.int32(30+thigh_angle+5))
                ard_RF3.publish(np.int32(180-thigh_angle-knee_angle+60-10))

                
                

                #------------------
                pubLF1.publish(-radians(adduction_angle))
                pubLF2.publish(-radians(thigh_angle))
                pubLF3.publish(-radians(knee_angle))
                pubRB1.publish(-radians(adduction_angle))
                pubRB2.publish(-radians(thigh_angle))
                pubRB3.publish(-radians(knee_angle))
                pubLB1.publish(-radians(adduction_angle))
                pubLB2.publish(-radians(thigh_angle))
                pubLB3.publish(-radians(knee_angle))
                pubRF1.publish(-radians(adduction_angle))
                pubRF2.publish(-radians(thigh_angle))
                pubRF3.publish(-radians(knee_angle))
                rate.sleep()
            ...
            #-------------------------------------------------------------------------------------
            #------------------------------------------------------------------------------------
            if self.rotate!=0 and self.halt != 1:
                T = self.stepLength/self.velocity
                LF=np.array([h,-2,z_off,1])
                RF=np.array([h,2,z_off,1])
                RB=np.array([h,2,z_off,1])
